...
export async function login(email, password) {
    ...
    if (!process.env.NEXT_PUBLIC_API_BASE_URL || !process.env.NEXT_PUBLIC_API_ADMIN_LOGIN_ROUTE) {
    ...
    try {
        ...
        const { status, data } = loginResponseJson
...
export async function updateSession(request) {
    ...
    if (!session) {
        ...
        return { status: 400, error: 'No session found' };
    }
